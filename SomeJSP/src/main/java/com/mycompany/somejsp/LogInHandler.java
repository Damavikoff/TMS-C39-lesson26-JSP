...
public class LogInHandler extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameterMap().containsKey("user") && request.getParameterMap().containsKey("password")) {
            ...
            String query = "select id from users where lower(login) = '" + user + "' and lower(password) = '" + password + "' order by 1 desc";
            List<Map<String, Object>> result = dbh.executeSelect(query);
            
            if (result == null || result.isEmpty()) {
                problems.add(result == null ? dbh.getLastError() : "Invalid credentials");
                //request.setAttribute("problems", problems);
                request.getSession().setAttribute("problems", problems);
                //this.getServletConfig().getServletContext().setAttribute("problems", problems);
                //request.getRequestDispatcher("/").forward(request, response);
                response.sendRedirect(request.getContextPath() + "/");
            } else {
                request.getSession().removeAttribute("problems");
                //this.getServletConfig().getServletContext().removeAttribute("problems");
